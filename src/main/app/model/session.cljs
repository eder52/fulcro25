...
(defn process-session-result [env error-message]
  (let [success? (uism/alias-value env :session-valid?)]
    (when success?
      (dr/change-route SPA ["pet"])  ;;;;;; CHANGED "MAIN" TO "PET!!!!" - should i route to a placeholder first?
       ;;;;;I GUESS HERE IS WHERE I LOAD THE RIGHT PET!?!??!
    
                 ;(df/load! SPA  :game/id ;(comp/get-ident this) ;[:demo/id 22]
                  ;GameCP ;{:target [:game/id]}
                  ;)
      )    
